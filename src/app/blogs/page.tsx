...
export default async function BlogsPage({ searchParams }: BlogsPageProps) {
  ...
  return (
    <div className="container-page py-8">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-text-primary mb-2">CCC Exam Blog</h1>
        <p className="text-text-muted">Expert tips, study guides and latest CCC exam updates</p>
      </div>
              ...
              <div className="flex-1 p-6 flex flex-col justify-center gap-3">
                <div className="flex items-center gap-2">
                  ...
                  {featured.blog_categories && (
                    <Badge variant="outline">{featured.blog_categories.title}</Badge>
                  )}
                </div>
                ...
                {featured.summary_en && (
                  <p className="text-text-secondary line-clamp-3">{featured.summary_en}</p>
                )}
              ...
              >
                ...
                <div className="p-5 flex flex-col flex-1 gap-2">
                  {blog.blog_categories && (
                    <Badge variant="primary" size="sm">{blog.blog_categories.title}</Badge>
                  )}
                  <h2 className="font-semibold text-text-primary group-hover:text-primary-600 transition-colors line-clamp-2 leading-snug">
                    {blog.title_en}
                  </h2>
                  {blog.summary_en && (
                    <p className="text-sm text-text-muted line-clamp-2">{blog.summary_en}</p>
                  )}
                  <div className="flex items-center gap-2 mt-auto pt-2 text-xs text-text-muted border-t border-border-subtle">
                    ...
                    <span className="ml-auto">{readingTime(blog.content_en)}</span>
                  </div>
                </div>
              </Link>
            ))}
